...
- **不覆盖**：
  - **Xiaomi/Redmi 反射**(`com.android.id.impl.IdProviderImpl`)——拦截它必须 hook "调用者敏感"的单参 `Class.forName(String)`,这会破坏 app 自身的类加载(例如 `androidx.startup`)并导致启动崩溃,故**有意不做**;小米原厂系统本身就有这个类。
  - **Huawei/Honor HMS**(`AdvertisingIdClient`)——依赖只存在于华为/荣耀原厂系统上的 HMS 类,AOSP 上本就不存在。
